...
while true; do
  ...
  echo "Container ID: $container_id_infra1"
  break
done
...
echo "Waiting for boot-org-1 to complete successfully..."
while true; do
  status=$(docker inspect --format='{{.State.Status}}' "$container_id_infra1")
  exit_code=$(docker inspect --format='{{.State.ExitCode}}' "$container_id_infra1")
...
cd org-3 && docker compose -f docker-compose-org3.yaml up -d